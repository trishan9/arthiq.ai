...
const addOfficialHeader = (doc: jsPDF, title: string, subtitle: string, config: OfficialDocumentConfig) => {
...
const addBusinessInfoSection = (doc: jsPDF, yPos: number, config: OfficialDocumentConfig): number => {
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  doc.text(`PAN Number: ${config.panNumber || '________________'}`, leftX, yPos + 20);
  doc.text(`Address: ${config.address || '________________'}`, leftX, yPos + 30);
  
  doc.text(`VAT Number: ${config.vatNumber || '________________'}`, rightX, yPos + 10);
  doc.text(`Registration No: ${config.registrationNumber || '________________'}`, rightX, yPos + 20);
  doc.text(`Fiscal Year: ${config.fiscalYear || getCurrentFiscalYear()}`, rightX, yPos + 30);
  
  return yPos + 45;
};

const addSignatureSection = (doc: jsPDF, yPos: number, config: OfficialDocumentConfig): void => {
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  doc.text(config.signatoryDesignation || 'Authorized Signatory', pageWidth - 55, yPos + 28, { align: 'center' });
  if (config.signatoryName) {
    doc.text(config.signatoryName, pageWidth - 55, yPos + 36, { align: 'center' });
  }
  
  // Date
  doc.text(`Date: ${formatDate(new Date())}`, pageWidth / 2, yPos + 28, { align: 'center' });
...
const addFooter = (doc: jsPDF, documentType: string) => {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount} | ${documentType} | Generated by Lekha`,
      pageWidth / 2,
      pageHeight - 10,
      { align: 'center' }
    );
...
export const generateVATReturnForm = (
  ...
  let yPos = addOfficialHeader(doc, 'VAT RETURN FORM', 'As per Inland Revenue Department Format', config);
  ...
  const outputData = [
    ['1', 'Taxable Sales (13%)', formatCurrency(metrics.totalRevenue), formatCurrency(outputVAT)],
  ...
  const inputData = [
    ['5', 'Taxable Purchases (13%)', formatCurrency(metrics.totalExpenses), formatCurrency(inputVAT)],
  ...
  const netData = [
    ...
    ['10', 'VAT Refund Claimed', formatCurrency(netVAT < 0 ? Math.abs(netVAT) : 0)],
  ...
  doc.setTextColor(80, 80, 80);
  doc.setFont('helvetica', 'normal');
  doc.text('I hereby declare that the information provided above is true and correct to the best of my knowledge.', 20, yPos);
...
export const generateTDSStatement = (
  ...
  let yPos = addOfficialHeader(doc, 'TDS DEPOSIT STATEMENT', 'Tax Deducted at Source Report', config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const tdsCategories = [
    { category: 'Salary & Wages', rate: '1% - 36%', amount: metrics.totalExpenses * 0.4, tds: metrics.totalExpenses * 0.4 * 0.15 },
    { category: 'Rent Payment', rate: '10%', amount: metrics.totalExpenses * 0.1, tds: metrics.totalExpenses * 0.01 },
    { category: 'Professional Services', rate: '15%', amount: metrics.totalExpenses * 0.15, tds: metrics.totalExpenses * 0.0225 },
    { category: 'Contract Payments', rate: '1.5%', amount: metrics.totalExpenses * 0.2, tds: metrics.totalExpenses * 0.003 },
  ...
  autoTable(doc, {
    startY: yPos,
    head: [['S.N.', 'Nature of Payment', 'TDS Rate', 'Payment Amount', 'TDS Deducted']],
    body: tdsData,
    theme: 'grid',
    headStyles: { fillColor: [26, 35, 126], textColor: 255, fontSize: 9 },
    styles: { fontSize: 9, cellPadding: 4 },
...
export const generateAnnualTaxReturn = (
  ...
  let yPos = addOfficialHeader(doc, 'ANNUAL TAX RETURN SUMMARY', 'Income Tax Computation Statement', config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const taxData = [
    ...
    ['12', 'Less: TDS Already Paid', formatCurrency(metrics.totalExpenses * 0.05)],
    ['13', 'Less: Advance Tax Paid', formatCurrency(0)],
    ['14', 'Net Tax Payable/(Refundable)', formatCurrency(Math.max(0, taxPayable - metrics.totalExpenses * 0.05))],
  ];
  ...
  doc.setTextColor(80, 80, 80);
  doc.setFont('helvetica', 'normal');
  const declaration = 'I/We hereby declare that the information provided in this return is true, correct and complete to the best of my/our knowledge and belief, and that no income has been concealed.';
...
export const generateProfitLossStatement = (
  ...
  let yPos = addOfficialHeader(doc, 'PROFIT & LOSS STATEMENT', `For the Fiscal Year ${config.fiscalYear || getCurrentFiscalYear()}`, config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const revenueData = metrics.revenueCategories.length > 0 
    ? metrics.revenueCategories.map((cat, idx) => [(idx + 1).toString(), cat.name, formatCurrency(cat.amount)])
  ...
  const expenseData = metrics.expenseCategories.length > 0
    ? metrics.expenseCategories.map((cat, idx) => [(idx + 1).toString(), cat.name, formatCurrency(cat.amount)])
    : [['1', 'Operating Expenses', formatCurrency(metrics.totalExpenses)]];
  
  expenseData.push(['', 'Total Expenses', formatCurrency(metrics.totalExpenses)]);
  ...
  doc.setFillColor(isProfit ? 34 : 239, isProfit ? 197 : 68, isProfit ? 94 : 68);
  doc.roundedRect(20, yPos, pageWidth - 40, 25, 3, 3, 'F');
  
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(14);
  doc.setFont('helvetica', 'bold');
  doc.text(`NET ${isProfit ? 'PROFIT' : 'LOSS'}:  ${formatCurrency(Math.abs(netProfit))}`, pageWidth / 2, yPos + 16, { align: 'center' });
...
export const generateBalanceSheet = (
  ...
  let yPos = addOfficialHeader(doc, 'BALANCE SHEET', `As at ${formatDate(new Date())}`, config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const liabilityData = [
    ...
    ['', 'Total Liabilities', formatCurrency(currentLiabilities + longTermLiabilities)],
    ['', 'EQUITY', ''],
  ...
  autoTable(doc, {
    ...
    didParseCell: function(data) {
      ...
      if (rowData && rowData[0] === '' && data.section === 'body') {
        data.cell.styles.fontStyle = 'bold';
        if (rowData[1] && (rowData[1].startsWith('TOTAL') || rowData[1].startsWith('Total'))) {
          data.cell.styles.fillColor = [240, 240, 240];
        }
      }
    }
...
export const generateBankLoanApplication = (
  ...
  let yPos = addOfficialHeader(doc, 'BUSINESS LOAN APPLICATION', 'Financial Summary for Banking Purposes', config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const financialData = [
    ...
    ['Net Annual Profit', formatCurrency(metrics.totalRevenue - metrics.totalExpenses)],
  ...
  doc.setFont('helvetica', 'normal');
  doc.text('This financial summary is generated based on actual documents uploaded to Lekha platform.', 25, yPos + 18);
  doc.text('Data accuracy depends on the quality and completeness of source documents provided.', 25, yPos + 25);
...
export const generateSSFReport = (
  ...
  let yPos = addOfficialHeader(doc, 'SOCIAL SECURITY FUND REPORT', 'Employee Contribution Statement', config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  doc.text('Note: SSF contribution rates as per Social Security Act 2074 (2017)', 20, yPos);
  doc.text('Employer: 20% | Employee: 11% | Total: 31% of basic salary', 20, yPos + 8);
...
export const generateAuditPackage = (
  ...
  let yPos = addOfficialHeader(doc, 'AUDIT-READY FINANCIAL PACKAGE', 'Comprehensive Financial Documentation', config);
  yPos = addBusinessInfoSection(doc, yPos, config);
  ...
  const summaryData = [
    ...
    ['Financial Health Score', `${metrics.financialHealthScore}/100`],
    ['Compliance Score', insights ? `${insights.summary.complianceScore}/100` : 'N/A'],
  ];
  ...
  yPos += 8;
  doc.text('Document Types: Bank Statements, Invoices, Receipts, Bills', 25, yPos);
  yPos += 8;
  ...
  if (insights) {
    ...
    if (insights.risks.length > 0) {
      ...
      const riskData = insights.risks.slice(0, 5).map(risk => [
