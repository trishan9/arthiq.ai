...
import { getFormattedKnowledgeBase, NEPAL_TAX_DATA, NEPAL_LABOR_LAW, calculateTaxEstimates, calculateSSF } from "../_shared/nepalKnowledge.ts";
  ...
  try {
    ...
    if (financialContext) {
      ...
      if (metrics) {
        ...
        businessContext += `- Total Revenue: NPR ${formatNum(metrics.totalRevenue)}\n`;
        businessContext += `- Total Expenses: NPR ${formatNum(metrics.totalExpenses)}\n`;
        businessContext += `- Net Profit: NPR ${formatNum(metrics.netProfit)}\n`;
        businessContext += `- Profit Margin: ${safeNumber(metrics.profitMargin).toFixed(1)}%\n`;
        businessContext += `- Outstanding Receivables: NPR ${formatNum(metrics.outstandingReceivables)}\n`;
        businessContext += `- Processed Documents: ${safeNumber(metrics.documentCount)}\n`;
        businessContext += `- Financial Health Score: ${safeNumber(metrics.financialHealthScore)}/100\n`;
        ...
        if (safeNumber(metrics.totalRevenue) >= vatThreshold) {
          ...
          businessContext += `\n⚠️ **VAT Warning**: Revenue at ${((safeNumber(metrics.totalRevenue) / vatThreshold) * 100).toFixed(0)}% of VAT threshold.\n`;
        }
        ...
        if (profit > 0) {
          ...
          businessContext += `- Estimated Annual Tax: NPR ${formatNum(taxEstimate.estimatedTax)}\n`;
          businessContext += `- Effective Tax Rate: ${taxEstimate.effectiveRate.toFixed(1)}%\n`;
        }
        ...
        if (metrics.monthlyData && Array.isArray(metrics.monthlyData) && metrics.monthlyData.length > 0) {
          ...
          metrics.monthlyData.forEach((m: any) => {
            if (m && m.month) {
              businessContext += `- ${m.month}: Revenue NPR ${formatNum(m.income)}, Expenses NPR ${formatNum(m.expenses)}\n`;
            }
          });
        }
        
        if (metrics.expenseCategories && Array.isArray(metrics.expenseCategories) && metrics.expenseCategories.length > 0) {
          businessContext += `\n### Expense Breakdown\n`;
          metrics.expenseCategories.forEach((c: any) => {
            if (c && c.name) {
              businessContext += `- ${c.name}: NPR ${formatNum(c.value)} (${safeNumber(c.percentage).toFixed(1)}%)\n`;
            }
          });
        }
      }
      ...
      if (documents && Array.isArray(documents) && documents.length > 0) {
        ...
        documents.slice(0, 10).forEach((doc: any) => {
          if (doc && doc.file_name) {
            businessContext += `- ${doc.file_name} (${doc.document_type || 'unknown'}, ${doc.status || 'pending'})\n`;
          }
        });
      }
      ...
      if (recentTransactions && Array.isArray(recentTransactions) && recentTransactions.length > 0) {
        ...
        recentTransactions.slice(0, 10).forEach((t: any) => {
          if (t && t.description) {
            businessContext += `- ${t.date || 'N/A'}: ${t.description} - NPR ${formatNum(t.amount)} (${t.type || 'unknown'})\n`;
          }
        });
      }
    }
...
`;
    ...
    const systemPrompt = `You are LekhaAI, a precise financial advisor for Nepal. Your responses must be CONCISE, DATA-DRIVEN, and ACTIONABLE.
...
${businessContext || "No documents uploaded. To get personalized advice, upload invoices, bank statements, or tax documents."}
...
You are their trusted accountant. Be direct, precise, and helpful.`;
    ...
    console.log("Sending request to Lovable AI with context length:", businessContext.length);
