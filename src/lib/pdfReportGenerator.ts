...
export const generateFinancialHealthReport = (
  ...
  const summaryData = [
    ...
    ['Net Profit/Loss', formatCurrency(metrics.totalRevenue - metrics.totalExpenses)],
  ...
  const scoreColor = metrics.financialHealthScore >= 70 ? [34, 197, 94] : 
                     metrics.financialHealthScore >= 40 ? [234, 179, 8] : [239, 68, 68];
  ...
  const healthStatus = metrics.financialHealthScore >= 70 ? 'Excellent' :
                       metrics.financialHealthScore >= 40 ? 'Good' : 'Needs Attention';
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount} | Generated by Lekha - Financial Management Platform`,
      pageWidth / 2,
...
export const generateVATReport = (
  ...
  doc.text('This is a summary report. Please verify with your accountant before filing.', 20, yPos);
  ...
  doc.setTextColor(128, 128, 128);
  doc.text(
    'Generated by Lekha - Financial Management Platform',
    pageWidth / 2,
    doc.internal.pageSize.getHeight() - 10,
    { align: 'center' }
  );
...
export const generateLoanApplicationReport = (
  ...
  const profitMargin = metrics.totalRevenue > 0 
    ? ((metrics.totalRevenue - metrics.totalExpenses) / metrics.totalRevenue * 100).toFixed(1)
    : '0.0';
  ...
  const summaryData = [
    ...
    ['Average Transaction Value', formatCurrency(metrics.averageTransactionValue)],
  ];
  ...
  if (metrics.monthlyData.length > 0) {
    ...
    const cashFlowData = metrics.monthlyData.slice(-6).map(m => [
      m.month,
      formatCurrency(m.income),
      formatCurrency(m.expenses),
      formatCurrency(m.income - m.expenses)
  ...
  yPos += 8;
  doc.text('Data extracted and verified using AI-powered document analysis.', 20, yPos);
  ...
  doc.setFontSize(9);
  doc.text('This document has been generated based on actual financial records uploaded to', 25, yPos + 20);
  doc.text('the Lekha platform. The data accuracy depends on the quality of source documents.', 25, yPos + 28);
  
  // Footer
  doc.setFontSize(8);
  doc.setTextColor(128, 128, 128);
  doc.text(
    'Generated by Lekha - Financial Management Platform | For Banking Purposes',
    pageWidth / 2,
    doc.internal.pageSize.getHeight() - 10,
    { align: 'center' }
  );
...
export const generateTransactionReport = (
  ...
  doc.text(`Net: ${formatCurrency(totalIncome - totalExpense)}`, pageWidth - 60, yPos);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(128, 128, 128);
    doc.text(
      `Page ${i} of ${pageCount} | Generated by Lekha`,
      pageWidth / 2,
      doc.internal.pageSize.getHeight() - 10,
      { align: 'center' }
    );
...
export const generateComplianceReport = (
  ...
  const scoreColor = insights.summary.complianceScore >= 70 ? [34, 197, 94] : 
                     insights.summary.complianceScore >= 40 ? [234, 179, 8] : [239, 68, 68];
  ...
  doc.setFontSize(9);
  const assessmentLines = doc.splitTextToSize(insights.summary.overallAssessment, 90);
  ...
  if (insights.taxObligations.length > 0) {
    ...
    const taxData = insights.taxObligations.map(tax => [
      ...
      tax.description.substring(0, 40) + (tax.description.length > 40 ? '...' : ''),
      tax.deadline || 'N/A'
    ]);
  ...
  if (insights.risks.length > 0 && yPos < 200) {
    ...
    const riskData = insights.risks.map(risk => [
      ...
      risk.title,
      risk.recommendation.substring(0, 50) + (risk.recommendation.length > 50 ? '...' : '')
    ]);
  ...
  if (insights.recommendations.length > 0 && yPos < 220) {
    ...
    const recData = insights.recommendations.slice(0, 5).map(rec => [
      ...
      rec.title,
      rec.expectedBenefit.substring(0, 40) + (rec.expectedBenefit.length > 40 ? '...' : '')
    ]);
  ...
  doc.setFontSize(8);
  doc.setTextColor(100, 100, 100);
  doc.text('DISCLAIMER: This report is for informational purposes only and does not constitute legal or tax advice.', 20, pageHeight - 25);
  doc.text('Please consult with a qualified accountant or tax professional for compliance verification.', 20, pageHeight - 18);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.setTextColor(128, 128, 128);
    doc.text(
      `Page ${i} of ${pageCount} | Generated by Lekha - Nepal Compliance Report`,
      pageWidth / 2,
      doc.internal.pageSize.getHeight() - 10,
      { align: 'center' }
    );
