...
import { getFormattedKnowledgeBase, NEPAL_TAX_DATA, NEPAL_LABOR_LAW, getThresholdAlerts } from "../_shared/nepalKnowledge.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Analyzing documents for regulation insights:", documents?.length || 0);
    ...
    const bankStatements = processedDocs.filter(d => d.document_type === "bank_statement");
    const taxDocs = processedDocs.filter(d => d.document_type === "tax_document");
    const pnlStatements = processedDocs.filter(d => d.document_type === "profit_loss");
    const balanceSheets = processedDocs.filter(d => d.document_type === "balance_sheet");
    ...
    if (metrics) {
      documentSummary += `\n## Financial Summary\n`;
      documentSummary += `- Total Revenue: NPR ${metrics.totalRevenue?.toLocaleString() || 0}\n`;
      documentSummary += `- Total Expenses: NPR ${metrics.totalExpenses?.toLocaleString() || 0}\n`;
      documentSummary += `- Net Profit: NPR ${((metrics.totalRevenue || 0) - (metrics.totalExpenses || 0)).toLocaleString()}\n`;
      documentSummary += `- VAT Collected: NPR ${metrics.vatCollected?.toLocaleString() || 0}\n`;
      documentSummary += `- Financial Health Score: ${metrics.financialHealthScore || 0}/100\n`;
      
      // Add threshold analysis
      const thresholdAlerts = getThresholdAlerts({ totalRevenue: metrics.totalRevenue });
      if (thresholdAlerts.length > 0) {
        documentSummary += `\n### Threshold Status\n`;
        thresholdAlerts.forEach(alert => {
          documentSummary += `- **${alert.threshold}**: ${alert.status.toUpperCase()} - ${alert.implication}\n`;
        });
      }
      ...
      if (metrics.expenseCategories?.length > 0) {
        ...
        metrics.expenseCategories.forEach(cat => {
          documentSummary += `- ${cat.name}: NPR ${cat.amount.toLocaleString()}\n`;
        });
      }
    }
    ...
    processedDocs.forEach(doc => {
      ...
      if (data.type === "bank_statement" && data.transactions) {
        ...
        txSample.forEach(tx => {
          transactionSummary += `- ${tx.date}: ${tx.description} | Debit: ${tx.debit || 0} | Credit: ${tx.credit || 0}\n`;
        });
      }

      if ((data.type === "invoice" || data.type === "receipt") && data.total_amount) {
        transactionSummary += `\n### ${data.type === "invoice" ? "Invoice" : "Receipt"} (${doc.file_name})\n`;
        transactionSummary += `- Amount: NPR ${data.total_amount}\n`;
        transactionSummary += `- VAT: NPR ${data.vat_amount || 0}\n`;
        transactionSummary += `- Vendor: ${data.vendor_name || data.merchant_name || "Unknown"}\n`;
      }

      if (data.type === "profit_loss") {
        transactionSummary += `\n### Profit & Loss Statement (${doc.file_name})\n`;
        transactionSummary += `- Period: ${data.period || "Not specified"}\n`;
        transactionSummary += `- Total Revenue: NPR ${data.total_revenue?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Total Expenses: NPR ${data.total_expenses?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Gross Profit: NPR ${data.gross_profit?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Net Profit: NPR ${data.net_profit?.toLocaleString() || "N/A"}\n`;
      }

      if (data.type === "balance_sheet") {
        transactionSummary += `\n### Balance Sheet (${doc.file_name})\n`;
        transactionSummary += `- Period: ${data.period || "Not specified"}\n`;
        transactionSummary += `- Total Assets: NPR ${data.total_assets?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Total Liabilities: NPR ${data.total_liabilities?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Total Equity: NPR ${data.total_equity?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Current Assets: NPR ${data.current_assets?.toLocaleString() || "N/A"}\n`;
        transactionSummary += `- Current Liabilities: NPR ${data.current_liabilities?.toLocaleString() || "N/A"}\n`;
      }
    });
...
4. Calculate VAT obligations based on ${(NEPAL_TAX_DATA.vat.standard_rate * 100)}% rate if turnover suggests registration (NPR ${NEPAL_TAX_DATA.thresholds.vat_registration.toLocaleString()})
